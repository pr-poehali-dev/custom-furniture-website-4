    ...
    <footer className="bg-charcoal-DEFAULT border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6 py-16 bg-slate-600">
        <div className="grid md:grid-cols-3 gap-12 mb-12">
          ...
          <div>
            <div className="font-oswald text-white/70 text-xs tracking-[0.2em] uppercase mb-5">Навигация</div>
            <div className="grid grid-cols-2 gap-2">
              {navLinks.map(link => (
                <button
                  key={link.label}
                  onClick={() => scrollTo(link.href)}
                  className="text-left font-golos text-white/80 hover:text-white text-sm transition-colors duration-200"
                >
                  {link.label}
                </button>
              ))}
          ...
          <div>
            <div className="font-oswald text-white/70 text-xs tracking-[0.2em] uppercase mb-5">Контакты</div>
            <div className="space-y-3">
              <a href="tel:+79783469108" className="flex items-center gap-3 font-golos text-white/80 hover:text-white text-sm transition-colors">
                <Icon name="Phone" size={14} className="text-gold-DEFAULT" />
                +7 978 346-91-08
              </a>
              <a href="mailto:market-wb8@yandex.ru" className="flex items-center gap-3 font-golos text-white/80 hover:text-white text-sm transition-colors">
                <Icon name="Mail" size={14} className="text-gold-DEFAULT" />
                market-wb8@yandex.ru
              </a>
              <div className="flex items-center gap-3 font-golos text-white/80 text-sm">
